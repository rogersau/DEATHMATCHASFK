class DAFRoundStats
{
	...
	static void Reset(string source = "")
	{
		...
		if (source != "")
			Print("DAFDeathmatch: resetting round K:D stats from " + source);
		else
			Print("DAFDeathmatch: resetting round K:D stats");
	}
	...
	static void ResetPlayer(PlayerIdentity identity, string source = "")
	{
		...
		if (source != "")
			Print("DAFDeathmatch: resetting player round K:D stats from " + source);
	}
