class DAFDMConfigReport
{
	...
	static void PrintReport(DAFDMSettings settings, DAFDMArenaRegistry arenas, DAFDMLoadoutRegistry loadouts, string source)
	{
		...
		PrintFormat("DAFDeathmatch: config flags (%1): autoRespawn=%2 playerRespawnCommand=%3 adminTestCommands=%4 spawnSafety=%5 tdmOutfitEnforcement=%6 lowPopWarmup=%7 forcePreferredFireMode=%8 weaponJamDisabled=1", source, settings.autoRespawn, settings.enablePlayerRespawnCommand, settings.enableAdminTestCommands, settings.enableSpawnSafety, settings.enforceTDMTeamOutfits, settings.enableLowPopWarmup, settings.forcePreferredWeaponFireMode);
