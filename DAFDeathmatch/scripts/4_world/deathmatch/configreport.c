class DAFDMConfigReport
{
	...
	private static int ValidateSettings(DAFDMSettings settings, DAFDMArenaRegistry arenas, DAFDMLoadoutRegistry loadouts)
	{
		...
		if (settings.spawnArenaWalls)
			warnings += WarnMissingClass(settings.arenaWallType, "arena wall type");

		if (hasTDMRound)
			warnings += ValidateTDMSettings(settings);

		warnings += ValidateVotingSettings(settings);
		warnings += ValidateSeasonSettings(settings);
	...
	private static int ValidateDiscordSettings(DAFDMSettings settings)
	{
		...
		warnings += ValidateDiscordEndpoint(settings.enableDiscordSeasonSummary, settings.discordSeasonSummaryWebhookUrl, "Discord season-summary");
		return warnings;
	}

	private static int ValidateSeasonSettings(DAFDMSettings settings)
	{
		if (!settings || !settings.enableSeasonScoring)
			return 0;

		int warnings = 0;
		if (settings.seasonKillPoints <= 0)
			warnings += Warn("season scoring is enabled but kill points are not positive");

		if (settings.seasonHeadshotBonusPoints < 0)
		...
		if (settings.seasonSummaryTopPlayers < 1)
			warnings += Warn("season summary top player count is below one");
	...
	private static int ValidateArenas(DAFDMSettings settings, DAFDMArenaRegistry arenas)
	{
		...
		foreach (DAFDMArena arena: allArenas)
		{
			...
			if (arena.IsRectangular() && (arena.GetXSize() <= 0 || arena.GetZSize() <= 0))
				warnings += Warn(string.Format("arena '%1' is rectangular but has invalid size x=%2 z=%3", name, arena.GetXSize(), arena.GetZSize()));
			else if (!arena.IsRectangular() && arena.GetRadius() <= 0)
				warnings += Warn(string.Format("arena '%1' has invalid radius %2", name, arena.GetRadius()));
